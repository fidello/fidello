...
public class PessoaFacade extends AbstractFacade implements Serializable {
	...
	public Pessoa autenticarPessoa(Pessoa pessoa) throws Exception {
		...
		try {
			...
			if (pessoa.getEmails() != null || pessoa.getNumeroSRF() != null) {
				...
				if (pessoa.getIdentificacaoTipo() == PessoaIdentificacaoTipoEnum.EMAIL
						.getCodigo()) {
					pessoaRetorno = this.detalharPessoaPorEmail(pessoa
							.getEmails());
				} else {
					pessoaRetorno = this
							.detalharPessoaPorNumeroSRF(
									PessoaIdentificacaoTipoEnum
											.getPessoaIdentificacaoTipoFromCodigo(pessoa
													.getIdentificacaoTipo()),
									pessoa.getNumeroSRF());
				}
	...
	public List<PessoaVO> buscarPessoasFisicas() throws Exception {
		...
		List<Pessoa> pessoas = pessoaDAO.buscarPessoasFisicas();
		List<PessoaVO> pessoasVO = new ArrayList<PessoaVO>();

		for (Pessoa pessoa : pessoas) {
			pessoasVO.add(new PessoaVO(pessoa.getId(), pessoa.getNome()));
	...
	private Pessoa detalharPessoaPorNumeroSRF(
			PessoaIdentificacaoTipoEnum pessoaIdentificacaoTipoEnum,
			String NumeroSRF) throws Exception {

		DocumentoTipoEnum documentoTipoEnum = DocumentoTipoEnum
				.getFromCodigo(pessoaIdentificacaoTipoEnum.getCodigo());

		return this.detalharPessoaPorNumeroSRF(documentoTipoEnum, NumeroSRF);

	}

	private Pessoa detalharPessoaPorNumeroSRF(DocumentoTipoEnum documentoTipo,
			String NumeroSRF) throws Exception {
	...
	private Pessoa detalharPessoaPorEmail(Collection<Email> emails)
			throws Exception {
	...
	private Pessoa cadastrarPessoa(Pessoa pessoa) throws Exception {
		...
		if (pessoaDAO.isNumeroSRFCadastrado(pessoa.getNumeroSRF(),
				DocumentoTipoEnum.getFromCodigo(pessoa.getDocumentoTipo()))) {
			throw new Exception(
		...
		try {
			...
			pessoa.getUsuario().setSenha(
					Utils.convertStringToMd5(pessoa.getUsuario().getSenha()));
